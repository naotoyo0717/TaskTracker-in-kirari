...
val_files_poisoned = load_file_paths(
    os.path.join(ACTIVATION_FILE_LIST_DIR, "val_poisoned_files_" + MODEL + ".txt")
)


# Model, Optimizer, and Loss Function Setup
model = ParallelConvProcessingModel(
    feature_dim=config.get("feature_dim"),
    num_layers=config.get("num_layers"),
    conv=config.get("conv"),
    layer_norm=config.get("layer_norm"),
    pool_first_layer=config.get("pool_first_layer"),
).cuda()
optimizer = torch.optim.Adam(model.parameters(), lr=config.get("learning_rate"))
scheduler = StepLR(
    optimizer,
    step_size=config.get("delay_lr_step"),
    gamma=config.get("delay_lr_factor"),
)
...
if config["restart"]:
    checkpoint_file = os.path.join(
        config.get("out_dir"), f"epoch_model_{start_epoch}_checkpoint.pth"
    )
...
def one_epoch_train(
    epoch_num, model, loss_function, optimizer, scheduler, train_files, config
):
    ...
    for i in range(0, len(train_files), config["files_chunk"]):
        ...
        dataset = ActivationsDatasetDynamicReturnText(
            chunk_files,
            config["activations"],
            TRAINING_TEXT_DIR,
        ...
        for data in tqdm(training_loader):
            ...
            step += 1
            with torch.no_grad():
                primary, clean, poisoned, text_batch = (
                    data[0].cuda(),
                    data[1].cuda(),
                    data[2].cuda(),
                    data[3],
                ...
                triplet_combinations = triplet_mining_unique(
                    primary_embs,
                    clean_embs,
                    poisoned_embs,
                    config["margin"],
            ...
            for k in range(0, len(triplet_combinations), batch_size):
                ...
                loss = loss_function(
                    anchor_emb_output, positive_emb_output, negative_emb_output, step
                )
                ...
                if global_counter_for_save % config["check_each"] == 0:
                    roc_auc = validation_ood(
                        model, val_files_clean, val_files_poisoned, config
                    )
...
def compute_distances_validation(model, val_files, config):
    ...
    for i in range(0, len(val_files), config.get("files_chunk")):
        ...
        val_loader = DataLoader(
            dataset, batch_size=config.get("batch_size"), shuffle=False
        )
        ...
        for data in val_loader:
            ...
            distances.extend(
                torch.norm(primary_embs - primary_with_text_embs, p=2, dim=-1)
                .cpu()
                .numpy()
            )
    return distances
...
def validation_ood(model, val_files_clean, val_files_poisoned, config):
    ...
    with torch.no_grad():  # Disable gradient computation
        ...
        distances_poisoned = compute_distances_validation(
            model, val_files_poisoned, config
        )

    distances_poisoned = process_val_data(
        config.get("ood_poisoned_file"), distances_poisoned
    )
...
for epoch in range(start_epoch, config.get("epochs")):
    one_epoch_train(
        epoch, model, triplet_loss, optimizer, scheduler, train_files, config
    )
